...
**Status:** APPROVED by Poormetheus (March 24, 2026) — with amendments on F0.1 and F0.3
...
**⚠️ Diagnostic fallback (from Poormetheus playtest):** The double-count bug makes monsters hit MORE, but Poormetheus observed 0/10 hits including nat 18 vs AC 14 — which suggests a second issue beyond the double-count. Possible causes: type coercion on `attackRoll.kept[0]` or `to_hit` being a string at runtime. **If all 5 unit tests pass but live combat still shows unexpected misses, add runtime logging to the monster attack path in `resolveAttack` (combat.ts ~line 110) to capture the actual values of `naturalRoll`, `totalAttackMod`, `targetAC`, and the hit/miss decision. Log as a `console.warn` so it's visible in server logs without a code change.**
